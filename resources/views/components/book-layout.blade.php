                    ...
                    <div class='col-3 p-0 d-flex justify-content-end align-items-center'>
                        ...
                        <div style='color:white;position:relative' class='mr-2'>
                            <div style='width:20px; height:20px;background-color:#23b85c; font-size:12px; border:none;
                                  border-radius:50%; position:absolute;right:2px;top:-2px; display: flex; justify-content: center; align-items: center;' id='cart-number-product'>
                            @if (session('cart'))
                                {{ count(session('cart')) }}
                            @else
                                0
        ...
        <script>
            $(document).ready(function(){
                $("#add-to-cart").click(function(){
                    id = "{{$data->id ?? ''}}"; // Thêm ?? '' để tránh lỗi nếu không ở trang chi tiết
